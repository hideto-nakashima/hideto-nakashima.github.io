...
function MakeFrame(){
	...
	ctx.moveTo(origin_X - N / 20, 1.75 * N);
	ctx.lineTo(origin_X + N / 20, 1.75 * N);
	ctx.closePath();
	ctx.stroke();
	/*平行移動量の情報を表示する*/
	ctx.font = "14px 'ＭＳ ゴシック'";
	ctx.fillText(' s = '+ ( 150 + trans_X) / 150 + ', t = ' + ( 150 + trans_Y) / 150, 20, 20);
	ctx.restore();
}
...
/* deg0 だけ回転された正方形を描く */
function DrawSquare(trans_x,trans_y){
	/* canvas要素のノードオブジェクト */
	var canvas = document.getElementById('canvas_plane');
	...
	ctx.lineTo(origin_X + 0.75 * N + trans_x, origin_Y);
	ctx.lineTo(origin_X + 0.75 * N + trans_x, origin_Y - 0.75 * N - trans_y);
	ctx.lineTo(origin_X, origin_Y - 0.75 * N - trans_y);
	ctx.closePath();
	ctx.globalAlpha = 0.5;
